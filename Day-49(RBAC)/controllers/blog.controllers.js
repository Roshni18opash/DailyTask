...
//fetch all data frm db & render in index.ejs
const index = async (req, res) => {
  try {
    let data = await blog.find();
    res.render("index", {
...
// form (only accessible for authenticated user)
...
// edit (fetch blog to populate the edit form)
//for only admin & manager
...
const editData = async (req, res) => {
  ...
  try {
    await blog.findByIdAndUpdate(
      id,
      { title, image, bname, type, date, dsc },
      { new: true },
    );
    res.redirect("/");
  } catch (error) {
